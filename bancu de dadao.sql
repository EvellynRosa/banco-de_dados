...
ALTER TABLE quarto ADD CONSTRAINT FKQuarto FOREIGN KEY (nome) REFERENCES hospede (nome) ON UPDATE CASCADE ON DELETE CASCADE

DROP TABLE quarto.66

INSERT INTO hospede (nome, genero, biotipo, altura) VALUES ('MIGUEL', 1, 1, 1.67)

UPDATE [tabela] SET [campo1 = valor1] [, campoN = valor] WHERE [condição]
...
SELECT nome, desconto + 0.05 nome desconto FROM quarto WHERE desconto IS NOT NULL

SELECT nome, saida - tempo FROM quarto FROM quarto

SELECT AVG(desconto), MAX(desconto), MIN(descontro) FROM quarto
